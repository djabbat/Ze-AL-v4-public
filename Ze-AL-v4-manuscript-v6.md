...
Pre‑registered power analysis: with $n=200$ distinct subjects, sampling error of Pearson $r$ is approximated via Fisher z‑transform with $\text{SE} = 1/\sqrt{n-3} \approx 0.0712$. Two‑sided $\alpha=0.05$ critical $z \approx 1.96$. For the pre‑registered effect‑size thresholds:
- $r \geq 0.30$ (truly independent behavioral target ceiling): power $= 0.991$
- $r \geq 0.25$: power $= 0.948$
